...
namespace SimpleSnake.GameObjects
{
    public class Snake
    {
        ...
        public bool IsMoving(Point direction)
        {
            ...
            bool isPointOfSnake = _snakeElements.Any(se => se.LeftX == _nextLeftX
            && se.TopY == _nextTopY);
            ...
            Point newSneakHead = new Point(_nextLeftX, _nextTopY);

            if (_wall.isPointOnWall(newSneakHead))
            {
                return false;
            }

            _snakeElements.Enqueue(newSneakHead);
            newSneakHead.Draw(SnakeSymbol);

            if (_foods[_foodIndex].IsFoodPoint(newSneakHead))
            {
